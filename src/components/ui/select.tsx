  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon asChild>
        <Icon
          name="chev-down"
          className={cn("right-4 data-[invalid=true]:text-red-500", caretStyle)}
        />
      ...
      >
        ...
        <SelectPrimitive.Viewport
          className={cn(
            "p-1",
            position === "popper" &&
              "h-[var(--radix-select-trigger-height)+24px]",
          )}
        >
          {children}
        </SelectPrimitive.Viewport>
...
function SelectItem({
  ...
  children,
  ...props
}: ComponentProps<typeof SelectPrimitive.Item> & { caretStyle?: string }) {
  return (
    <SelectPrimitive.Item
      data-slot="select-item"
      className={cn(
        "focus:bg-blue-300/15 focus:text-accent-foreground relative flex w-full cursor-default items-center rounded py-1.5 ps-8 pe-2 text-sm outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50",
        className,
      )}
      {...props}
    >
      <span className="absolute start-2 flex size-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <Icon name="checkmark-circle" className={cn("right-4", caretStyle)} />
        </SelectPrimitive.ItemIndicator>
      </span>
...
function SelectScrollUpButton({
  ...
  return (
    <SelectPrimitive.ScrollUpButton
      ...
      {...props}
    >
      <Icon name="chev-up" className={cn("", caretStyle)} />
    </SelectPrimitive.ScrollUpButton>
  );
}
...
function SelectScrollDownButton({
  ...
  return (
    <SelectPrimitive.ScrollDownButton
      ...
      {...props}
    >
      <Icon name="chev-down" className={cn("", caretStyle)} />
    </SelectPrimitive.ScrollDownButton>
  );
}
